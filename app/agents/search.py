...
async def search_single_intent(
    client: PerplexityClient, intent: Dict[str, str], delay: float = 0
) -> Dict[str, Any]:
    ...
    async with semaphore:
        ...
        try:
            result = await client.ask(
                question=query,
                system_prompt=(
                    "Ты - аналитик бизнес-аналитики и data-mining. Найди и кратко изложи информацию по запросу. "
...
async def search_agent(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not search_intents:
        logger.warning(
            "Search Agent: no search intents provided", component="search_agent"
        )
        return {
            **state,
            "search_results": [],
    ...
    if not client.is_configured():
        logger.error(
            "Search Agent: Perplexity API key not configured", component="search_agent"
        )
        return {
            **state,
            "search_results": [],
    ...
    return {**state, "search_results": search_results, "current_step": "analyzing"}
