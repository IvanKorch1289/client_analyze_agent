...
if source in ["dadata", "casebook", "infosphere"]:
    ...
    with st.form(f"{source}_form"):
        ...
        submitted = st.form_submit_button(
            "🔍 Получить данные", use_container_width=True, type="primary"
        )
    ...
    st.caption(
        "AI-powered поиск информации о компании. Возвращает структурированный ответ с цитатами."
    )
    ...
    with st.form("perplexity_form"):
        ...
        with col1:
            inn = st.text_input("ИНН компании", placeholder="7707083893", max_chars=12)
        ...
        submitted = st.form_submit_button(
            "🔍 Поиск", use_container_width=True, type="primary"
        )
    ...
    st.caption(
        "Веб-поиск с глубоким анализом. Возвращает релевантные результаты с ответом AI."
    )
    ...
    if submitted and inn and search_query:
        ...
        with st.spinner("🔍 Tavily ищет..."):
            try:
                result = client.search_tavily(
                    inn, search_query, max_results=max_results
                )
