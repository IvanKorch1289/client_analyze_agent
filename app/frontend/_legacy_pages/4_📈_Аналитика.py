...
try:
        ...
        with col1:
            ...
            if chart_data:
                df = pd.DataFrame.from_dict(
                    chart_data, orient="index", columns=["Количество"]
                )
    ...
    if timeline:
        ...
        # Create DataFrame
        df = pd.DataFrame(
            {"Дата": dates, "Количество анализов": counts, "Средний риск": avg_risks}
        )
...
with st.expander("📊 Расширенная аналитика"):
    ...
    try:
        ...
        if timeline_30d:
            ...
            # Create DataFrame
            df = pd.DataFrame({"Дата": dates, "Средний риск": avg_risks})
            df = df.set_index("Дата")
