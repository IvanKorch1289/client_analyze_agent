...
try:
    ...
    if metrics_data.get("status") == "success":
        ...
        # Calculate totals
        total_requests = sum(
            m.get("total_requests", 0) for m in metrics.values() if isinstance(m, dict)
        )
        total_errors = sum(
            m.get("errors", 0) for m in metrics.values() if isinstance(m, dict)
        )
        ...
        if stats:
            with st.container():
                ...
                for idx, level in enumerate(levels):
                    with cols[idx]:
                        st.metric(
                            f"{icons.get(level, '')} {level}", stats.get(level, 0)
                        )
        ...
        if breakers:
            for service, cb_info in breakers.items():
                ...
                with col1:
                    ...
                    icon = (
                        "🟢" if state == "closed" else "🔴" if state == "open" else "🟡"
                    )
                ...
                with col3:
                    if st.button("🔄 Сброс", key=f"reset_cb_{service}"):
                        try:
                            result = client.post(
                                f"/utility/circuit-breakers/{service}/reset", json={}
                            )
                            if result.get("status") == "success":
...
with col1:
    st.link_button(
        "📖 Swagger UI",
        f"{st.session_state.get('api_base_url', 'http://localhost:8000/api/v1')}/docs",
    )
    st.link_button(
        "📄 OpenAPI JSON",
        f"{st.session_state.get('api_base_url', 'http://localhost:8000/api/v1')}/openapi.json",
    )

with col2:
    st.link_button(
        "📡 AsyncAPI HTML",
        f"{st.session_state.get('api_base_url', 'http://localhost:8000/api/v1')}/utility/asyncapi",
    )
    st.link_button(
        "📄 AsyncAPI JSON",
        f"{st.session_state.get('api_base_url', 'http://localhost:8000/api/v1')}/utility/asyncapi.json",
    )
