...
with st.form("analysis_form"):
    ...
    with col2:
        ...
        if st.form_submit_button("🚀 Анализировать", use_container_width=True, type="primary"):
            submitted = True
        else:
            submitted = False
...
if st.session_state.get("last_response"):
    ...
    with col3:
        ...
        st.metric("Время", timestamp[:19] if timestamp and len(timestamp) > 19 else timestamp)
            ...
            if report:
                ...
                if risk_assessment:
                    ...
                    with col1:
                        ...
                        st.markdown(render_risk_badge(risk_level, risk_score), unsafe_allow_html=True)
                ...
                if findings:
                    ...
                    for finding in findings:
                        with st.expander(f"**{finding.get('category', 'Категория')}**"):
                            st.markdown(f"**Sentiment:** {finding.get('sentiment', 'neutral')}")
