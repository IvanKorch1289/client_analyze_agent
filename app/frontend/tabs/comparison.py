...
def render_comparison_tab(api: ApiClient):
    ...
    **Сравните два анализа клиента side-by-side** для отслеживания изменений
    ...
    if not _render_search_step(api):
        return

    _render_select_step(api)
    _render_comparison_results()


def _render_search_step(api: ApiClient) -> bool:
    """Step 1: Search by INN. Returns True if reports found."""
    st.divider()
...
def _format_report_option(report) -> str:
    """Format report for selectbox display."""
...
def _render_select_step(api: ApiClient) -> None:
    """Step 2: Select two reports and compare."""
    if not st.session_state.get("comparison_reports"):
        return
    ...
    report_options = {_format_report_option(r): r for r in reports}
...
def _render_comparison_results() -> None:
    ...
    _render_risk_comparison(diff)
    _render_factors_comparison(diff)
    _render_findings_comparison(diff)
    _render_report_info(report1_info, report2_info)
...
def _render_summary_metrics(summary: dict) -> None:
    ...
    with col1:
        st.metric("Всего изменений", summary.get("total_changes", 0), help="Количество метрик, которые изменились")

    with col2:
        st.metric("Улучшения", len(improvements), delta=len(improvements) if improvements else None, delta_color="normal", help="Позитивные изменения")

    with col3:
        st.metric("Ухудшения", len(regressions), delta=-len(regressions) if regressions else None, delta_color="inverse", help="Негативные изменения")
...
def _render_risk_comparison(diff: dict) -> None:
    """Render risk score comparison expander."""
    st.divider()
    ...
    with st.expander("⚠️ **Риск-оценка**", expanded=True):
        ...
        with col1:
            st.markdown("**Отчёт A**")
            st.metric("Risk Score", diff["risk_score"]["old"], help=f"Уровень: {diff['risk_level']['old']}")

        with col2:
            st.markdown("**Отчёт B**")
            st.metric("Risk Score", diff["risk_score"]["new"], delta=diff["risk_score"]["delta"], delta_color="inverse", help=f"Уровень: {diff['risk_level']['new']}")
...
def _render_factors_comparison(diff: dict) -> None:
    """Render risk factors comparison expander."""
...
def _render_findings_comparison(diff: dict) -> None:
    """Render findings and data quality comparison expander."""
    with st.expander("📋 **Находки и рекомендации**"):
        col1, col2 = st.columns(2)

        with col1:
            st.markdown("**Findings (находки)**")
            st.metric("Отчёт A", diff["findings_count"]["old"])
            st.metric("Отчёт B", diff["findings_count"]["new"], delta=diff["findings_count"]["delta"])

        with col2:
            st.markdown("**Data Quality**")
            st.metric("Отчёт A", f"{diff['data_quality']['old']}%")
            st.metric("Отчёт B", f"{diff['data_quality']['new']}%", delta=f"{diff['data_quality']['delta']}%", delta_color="normal")


def _render_report_info(report1_info: dict, report2_info: dict) -> None:
    """Render additional report information expander."""
