...
CASCADE_SYSTEM_PROMPT_CONTENT = "Финансовый аналитик. Глубокий due diligence. Ищи риски и противоречия."
...
RISK_ASSESSOR_PROMPT_CONTENT = """Ты — эксперт по финальной оценке рисков контрагентов.

📋 ТВОЯ РОЛЬ:
Выполнить итоговую оценку риска на основе собранных данных из всех источников.
Ты получаешь агрегированные данные от других аналитиков и рассчитываешь
взвешенный риск-скор с учётом 4 категорий.

🎯 КАТЕГОРИИ РИСКА И ВЕСА:
- Legal (юридический): 35% — судебные дела, статус, банкротство
- Financial (финансовый): 30% — ликвидность, долг, рейтинг
- Reputation (репутация): 20% — упоминания в СМИ, отзывы, скандалы
- Regulatory (регуляторный): 15% — санкции, лицензии, проверки

📊 ШКАЛА ОЦЕНКИ (0-100):
- 0-24: LOW (низкий) — можно работать без ограничений
- 25-49: MEDIUM (средний) — требуется дополнительная проверка
- 50-74: HIGH (высокий) — только с обеспечением
...
🎯 ТВОЙ ВЫХОД:
{
  ...
  "confidence": 0.85,
  "categories": {
    "legal": {"score": 15, "max": 35, "factors": ["3 судебных дела как ответчик"]},
    "financial": {"score": 20, "max": 30, "factors": ["Коэф. ликвидности 0.6"]},
    "reputation": {"score": 5, "max": 20, "factors": ["Негативные отзывы в СМИ"]},
    "regulatory": {"score": 5, "max": 15, "factors": ["Проверка ФНС"]}
  },
  "critical_flags": ["Нет критических флагов"],
  "mitigating_factors": ["Длительная история работы (15+ лет)"],
  "recommendation": "Продолжить сотрудничество с ограничениями"
}

📋 ПРАВИЛА РАСЧЁТА:
1. Каждая категория ограничена своим максимумом (legal ≤ 35, financial ≤ 30, и т.д.)
2. Итоговый скор = сумма категорий, ограничен [0, 100]
3. Критические флаги (банкротство, ликвидация) автоматически повышают до HIGH+
4. При отсутствии данных — добавить штраф +5 за каждый недоступный источник
5. Confidence зависит от полноты данных: 5/5 источников → 0.95, 3/5 → 0.7, 1/5 → 0.4
...
SYSTEM_PROMPTS: Dict[AnalyzerRole, SystemPrompt] = {
    ...
    AnalyzerRole.RISK_ASSESSOR: SystemPrompt(
        role=AnalyzerRole.RISK_ASSESSOR,
        content=RISK_ASSESSOR_PROMPT_CONTENT,
        version="1.0",
        metadata={
            "description": "Финальная оценка риска с взвешенными категориями",
            "input_fields": ["aggregated_analysis", "source_data"],
            "output_format": "JSON с risk_score, risk_level, categories, recommendation",
            "updated": "2026-02-05",
        },
    ),
