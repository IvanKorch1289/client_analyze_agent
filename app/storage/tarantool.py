...
async def save_thread_to_tarantool(thread_id: str, data: Dict[str, Any]):
    """
    Сохраняет состояние графа (или любой объект) в Tarantool как персистентный тред.
    Поддерживает любые входные данные — автоматически нормализует.
    """
    try:
        ...
        # Сериализуем messages, если есть
        processed_messages = []
        messages = data.get("messages", [])
        for msg in messages:
            try:
                if hasattr(msg, "dict") and callable(msg.dict):
                    # Pydantic-совместимый объект
        ...
        # Формируем запись для Tarantool
        record = {
            "messages": processed_messages,
            "created_at": data.get("created_at", time.time()),
            "input": str(data.get("input", "")),
            "thread_id": normalized_id,
            "final_state": data,  # ← Сохраняем всё состояние целиком
        }

        # Сохраняем
        await client.set_persistent(f"thread:{normalized_id}", record)

        logger.info(f"Thread saved to Tarantool: thread:{normalized_id}")

    except Exception as e:
        logger.error(f"Ошибка при сохранении в Tarantool: {e}")
