...
class ThreadsRepository(BaseRepository[Dict[str, Any]]):
    ...
    async def get(self, thread_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Thread get error for {thread_id}: {e}",
    ...
    async def save_thread(
        ...
        if existing:
            ...
            return await self.update(thread_id, {
                "thread_data": thread_data,
    ...
    async def update(self, thread_id: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Thread update error for {thread_id}: {e}",
                component="threads_repo"
            )
            return False
    ...
    async def delete(self, thread_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Thread delete error for {thread_id}: {e}",
                component="threads_repo"
            )
            return False
    ...
    async def list(
        ...
        try:
            ...
            logger.error(
                f"Threads list error: {e}",
                component="threads_repo"
            )
            return []
    
    async def list_threads_by_inn(
        self,
        inn: str,
        limit: int = 50
    ) -> List[Dict[str, Any]]:
    ...
    async def list_threads_by_client_name(
        self,
        client_name: str,
        limit: int = 50
    ) -> List[Dict[str, Any]]:
        ...
        logger.debug(
            f"Get threads by client name: {client_name}",
            component="threads_repo"
        )
