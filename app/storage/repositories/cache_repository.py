...
class CacheRepository(BaseRepository[Dict[str, Any]]):
        ...
        try:
            ...
            if isinstance(result, dict) and "ttl" in result:
                if result["ttl"] < time.time():
                    ...
                    logger.debug(f"Cache key expired and deleted: {key}", component="cache_repo")
                    return None

            return result

        except Exception as e:
            logger.error(f"Cache get error for key {key}: {e}", component="cache_repo")
            return None
    ...
    async def set_with_ttl(self, key: str, value: Any, ttl: int = 3600, source: str = "api") -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Cache set error for key {key}: {e}", component="cache_repo")
            return False
    ...
    async def update(self, key: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Cache update error for key {key}: {e}", component="cache_repo")
            return False
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Cache delete error for key {key}: {e}", component="cache_repo")
            return False
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"hits": 0, "misses": 0, "hit_rate_percent": 0, "error": str(e)}
    ...
    async def get_stats_by_source(self, source: str) -> Dict[str, int]:
        ...
        """
        # TODO: Implement через прямое обращение к Tarantool
        logger.debug(f"Stats by source requested: {source}", component="cache_repo")
