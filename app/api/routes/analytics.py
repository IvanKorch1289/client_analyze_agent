...
async def get_risk_trends(
    ...
    try:
        ...
        if len(timeline) >= 2:
            ...
            first_avg = sum(d["avg_risk"] for d in first_week if d["count"] > 0) / max(1, len([d for d in first_week if d["count"] > 0]))
            last_avg = sum(d["avg_risk"] for d in last_week if d["count"] > 0) / max(1, len([d for d in last_week if d["count"] > 0]))
