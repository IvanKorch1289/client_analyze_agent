...
class TestP0_2_WaitForRemoval:
    ...
    async def test_dadata_still_has_wait_for(self):
        ...
        with patch("app.agents.data_collector.fetch_from_dadata") as mock_fetch:
            ...
            assert result["success"] is False, "DaData должен упасть по таймауту 30s"
            assert result["error"] == "Timeout", "Ошибка должна быть Timeout"
    ...
    async def test_infosphere_no_wait_for(self):
        ...
        with patch("app.agents.data_collector.fetch_from_infosphere") as mock_fetch:
            ...
            try:
                result = await asyncio.wait_for(_fetch_infosphere_wrapper("7736050003"), timeout=150)

                # Должен успешно завершиться
                assert result["success"] is True, "InfoSphere должен успешно завершиться без wait_for"
                assert "test" in str(result.get("data")), "Данные должны быть получены"
    ...
    async def test_casebook_no_wait_for(self):
        ...
        with patch("app.agents.data_collector.fetch_from_casebook") as mock_fetch:
            ...
            try:
                result = await asyncio.wait_for(_fetch_casebook_wrapper("7736050003"), timeout=150)

                # Должен успешно завершиться
                assert result["success"] is True, "Casebook должен успешно завершиться без wait_for"
                assert isinstance(result.get("data"), list), "Данные должны быть списком дел"
    ...
    async def test_error_handling_preserved(self):
        ...
        with patch("app.agents.data_collector.fetch_from_infosphere") as mock_fetch:
            ...
            assert result["success"] is False, "При ошибке success должен быть False"
            assert "API Error" in result["error"], "Ошибка должна быть сохранена"
...
class TestP0_2_Integration:
    ...
    async def test_casebook_can_handle_long_pagination(self):
        ...
        assert result["success"] is True, f"Casebook должен успешно завершиться, error: {result.get('error')}"
