...
class TestP0_1_TimeoutConstants:
    ...
    def test_infosphere_read_timeout_increased(self):
        ...
        assert AppTimeoutConfig.INFOSPHERE_READ == 360.0, "InfoSphere READ timeout должен быть 360s (6 минут)"

    def test_casebook_read_timeout_increased(self):
        """Проверка что таймаут Casebook увеличен до 360s."""
        assert AppTimeoutConfig.CASEBOOK_READ == 360.0, "Casebook READ timeout должен быть 360s (6 минут)"

    def test_dadata_timeout_unchanged(self):
        """Проверка что таймаут DaData не изменился (не требует длительной обработки)."""
        assert AppTimeoutConfig.DADATA_READ == 30.0, "DaData READ timeout должен остаться 30s"
...
class TestP0_1_HttpClientConfig:
    ...
    async def test_http_client_uses_new_timeouts(self):
        ...
        assert infosphere_timeout.read == 360.0, "InfoSphere должен использовать read timeout 360s"
        ...
        assert casebook_timeout.read == 360.0, "Casebook должен использовать read timeout 360s"
    ...
    async def test_circuit_breaker_timeout_increased(self):
        ...
        assert casebook_cb.timeout == 400.0, "Casebook circuit breaker timeout должен быть 400s (больше read timeout)"
    ...
    async def test_dadata_config_unchanged(self):
        ...
        assert dadata_timeout.read == 30.0, "DaData read timeout должен остаться 30s"

        dadata_cb = dadata_config["circuit_breaker"]
        assert dadata_cb.timeout == 30.0, "DaData circuit breaker timeout должен остаться 30s"
...
class TestP0_1_Integration:
    ...
    async def test_infosphere_long_request(self):
        ...
        assert result.get("status") == "success", f"InfoSphere должен успешно выполниться, error: {result.get('error')}"
