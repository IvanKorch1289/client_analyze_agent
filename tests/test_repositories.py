...
async def test_reports_repository_ttl():
    ...
    # Создаем отчет
    report_id = await repo.create({"inn": "1234567890", "client_name": "Test", "report_data": {}})
...
async def test_threads_repository_list():
    ...
    for i in range(5):
        await repo.save_thread(thread_id=f"thread_{i}", thread_data={"index": i}, client_name=f"Client {i}")
...
async def test_real_thread_operations():
    ...
    try:
        ...
        # Update
        updated = await repo.update(thread_id, {"client_name": "Updated Client"})
        assert updated is True
