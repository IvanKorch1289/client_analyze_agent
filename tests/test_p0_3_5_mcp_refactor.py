...
class TestP0_3_5_DuplicateRemoval:
    ...
    def test_shared_prompts_deleted(self):
        ...
        assert not file_path.exists(), "Дубликат app/agents/shared/prompts.py должен быть удалён"
    ...
    def test_agents_import_from_mcp(self):
        ...
        assert "get_system_prompt" in analyzer_content, "Analyzer должен использовать get_system_prompt"
...
class TestP0_3_5_SystemPrompts:
    ...
    def test_get_system_prompt_exists(self):
        ...
        assert callable(get_system_prompt), "get_system_prompt должна быть функцией"
    ...
    def test_all_roles_have_prompts(self):
        ...
        for role in required_roles:
            assert role in SYSTEM_PROMPTS, f"Роль {role} должна иметь системный промпт"

            prompt = get_system_prompt(role)
            assert len(prompt) > 100, f"Промпт для {role} не должен быть пустым"
    ...
    def test_prompts_are_in_russian(self):
        ...
        assert any(ord(c) > 1000 for c in prompt), "Промпт должен содержать русские символы"
    ...
    def test_format_dadata_helper(self):
        ...
        assert "ГАЗПРОМ" in formatted, "Должно содержать название компании"
        assert "ACTIVE" in formatted, "Должно содержать статус"
...
class TestP0_3_5_MCPPrompts:
    ...
    def test_mcp_has_system_prompts(self):
        ...
        assert "system_prompt_orchestrator" in mcp_main_content, "MCP должен экспортировать промпт оркестратора"

        assert "system_prompt_report_analyzer" in mcp_main_content, "MCP должен экспортировать промпт анализатора"

        assert "system_prompt_registry_analyzer" in mcp_main_content, (
            "MCP должен экспортировать промпт реестрового анализатора"
        )

        assert "system_prompt_web_analyzer" in mcp_main_content, "MCP должен экспортировать промпт веб-аналитика"
...
class TestP0_3_5_BestPractices:
    ...
    def test_best_practices_file_created(self):
        ...
        assert file_path.exists(), "best_practices.py должен существовать"
    ...
    def test_best_practices_content(self):
        ...
        assert "ОБЯЗАТЕЛЬНЫЕ ПРОВЕРКИ" in ANALYSIS_BEST_PRACTICES, "Должен содержать раздел обязательных проверок"

        assert "ЕГРЮЛ" in ANALYSIS_BEST_PRACTICES, "Должен упоминать ЕГРЮЛ"

        assert "ФОРМУЛА РАСЧЁТА" in ANALYSIS_BEST_PRACTICES.upper(), "Должен содержать формулы расчёта"

        assert "КРИТИЧЕСКИЕ ФЛАГИ" in ANALYSIS_BEST_PRACTICES.upper(), "Должен описывать критические флаги"

        # Проверяем длину (должен быть comprehensive)
        assert len(ANALYSIS_BEST_PRACTICES) > 5000, "Best practices должны быть подробными (>5000 символов)"
    ...
    def test_api_examples_content(self):
        ...
        assert "DaData" in API_USAGE_EXAMPLES, "Должны быть примеры DaData"

        assert "Casebook" in API_USAGE_EXAMPLES, "Должны быть примеры Casebook"

        assert "InfoSphere" in API_USAGE_EXAMPLES, "Должны быть примеры InfoSphere"

        assert "Perplexity" in API_USAGE_EXAMPLES, "Должны быть примеры Perplexity"

        assert "Tavily" in API_USAGE_EXAMPLES, "Должны быть примеры Tavily"

        # Проверяем наличие кода
        assert "```python" in API_USAGE_EXAMPLES or "```xml" in API_USAGE_EXAMPLES, "Должны быть code examples"

    def test_mcp_exports_best_practices(self):
        """Проверка что MCP экспортирует best practices как resource."""
        mcp_main_content = Path("app/mcp_server/main.py").read_text()

        assert "best_practices_resource" in mcp_main_content, "MCP должен экспортировать best_practices resource"

        assert "app://best-practices" in mcp_main_content, "MCP должен иметь URI app://best-practices"

        assert "api_examples_resource" in mcp_main_content, "MCP должен экспортировать api_examples resource"

        assert "api_specs_resource" in mcp_main_content, "MCP должен экспортировать api_specs resource"
