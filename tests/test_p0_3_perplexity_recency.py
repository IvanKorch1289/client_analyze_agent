...
class TestP0_3_RecencyFilter:
    ...
    async def test_recency_filter_is_passed(self):
        ...
        with patch("langchain_openai.ChatOpenAI") as mock_chat:
            ...
            assert "model_kwargs" in call_kwargs, "model_kwargs должен быть передан"
    ...
    async def test_fallback_when_not_supported(self):
        ...
        with patch("langchain_openai.ChatOpenAI") as mock_chat:
            ...
            mock_instance_error.ainvoke.side_effect = Exception("Parameter search_recency_filter is not supported")
            ...
            assert result["success"] is True, "Fallback должен привести к успеху"
            assert "Success" in result["content"], "Контент должен быть от второго вызова"

            # ChatOpenAI должен быть вызван дважды
            assert mock_chat.call_count == 2, "ChatOpenAI должен быть вызван дважды (original + fallback)"

            # Второй вызов не должен иметь model_kwargs
            second_call_kwargs = mock_chat.call_args_list[1][1]
            assert "model_kwargs" not in second_call_kwargs or second_call_kwargs.get("model_kwargs") is None, (
                "Второй вызов не должен иметь model_kwargs"
            )
    ...
    async def test_other_errors_are_propagated(self):
        ...
        with patch("langchain_openai.ChatOpenAI") as mock_chat:
            ...
            assert result["success"] is False, "Другие ошибки должны приводить к failure"
            assert "rate limit" in result["error"].lower(), "Ошибка должна содержать исходное сообщение"
...
class TestP0_3_Integration:
    ...
    async def test_real_api_with_year_filter(self):
        ...
        assert result["success"] is True, f"Запрос должен быть успешным, error: {result.get('error')}"
